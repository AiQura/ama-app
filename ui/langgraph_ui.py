...
class LangGraphUI:
    ...
    def render_langgraph_section(self, current_user: Optional[User] = None) -> None:
        ...
        if "OPENAI_API_KEY" not in os.environ:
            st.warning("⚠️ OpenAI API key is not set in environment variables. Please ask your administrator to configure it.")
            return
        ...
        with col1:
            ...
            if self.file_service and current_user:
                # Get user files
                user_files = self.file_service.get_user_files(current_user.user_id)
                
                if not user_files:
                    st.info("No files available. Upload files from the sidebar.")
                else:
                    # Create a multiselect for files
                    file_options = {f"{file.name}": file for file in user_files}
                    ...
                    if selected_files:
                        st.success(f"Selected {len(selected_files)} files for RAG")
            else:
        ...
        with col2:
            ...
            if self.link_service and current_user:
                # Get user links
                user_links = self.link_service.get_user_links(current_user.user_id)
                ...
                if not user_links:
                    ...
                    selected_links = [link_options[url] for url in selected_link_urls]
                    
                    if selected_links:
                        st.success(f"Selected {len(selected_links)} links for RAG")
            else:
        ...
        query = st.text_area("Enter your question about the selected content", height=100)
        
        # Submit button
        if st.button("Run Query with LangGraph"):
            if query:
                if not selected_files and not selected_links:
                    st.warning("You haven't selected any files or links. LangGraph will use default sources or web search.")
